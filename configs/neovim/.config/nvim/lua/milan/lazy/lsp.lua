    ...
    config = function()
        ...
        lsp.on_attach(function(client, bufnr)
            ...
            vim.keymap.set("n", "<leader>f", function() vim.lsp.buf.format({ async = true }) end,
                vim.tbl_deep_extend("force", opts, { desc = "Format Document" }))

            -- Formatar antes de salvar para linguagens específicas
            local filetype = vim.api.nvim_buf_get_option(bufnr, "filetype")
            local format_on_save = { "python", "go", "java", "javascript", "typescript", "lua" }

            if vim.tbl_contains(format_on_save, filetype) then
                vim.api.nvim_create_autocmd("BufWritePre", {
                    buffer = bufnr,
                    callback = function()
                        vim.lsp.buf.format({ async = false }) -- Formatação síncrona antes de salvar
                    end,
                })
            end
